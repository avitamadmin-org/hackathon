...
function Toggle({
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-5 w-9 shrink-0 rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20
...
function FieldRow({
  ...
  return (
    <div className="flex items-start gap-4 py-5 border-b border-slate-100 last:border-0 group hover:bg-blue-50/30 px-2 -mx-2 rounded-lg transition-colors">
      ...
      {field.custom ? (
        <button
          type="button"
          onClick={onDelete}
          className="mt-0.5 p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition shrink-0"
        >
        ...
        <div className="px-6 py-5 space-y-4 bg-slate-50/40">
          <div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none text-sm border border-slate-200 rounded-xl px-3 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition bg-white text-slate-700 pr-8"
              >
                {FIELD_TYPES.map((t) => (
          ...
          <div>
            ...
            <input
              ...
              autoFocus
              className="w-full text-sm border border-slate-200 rounded-xl px-3 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition bg-white placeholder:text-slate-400"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full text-sm border border-slate-200 rounded-xl px-3 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition bg-white placeholder:text-slate-400 resize-none"
            />
          </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-white border-t border-slate-100">
          <label className="flex items-center gap-2.5 cursor-pointer select-none">
            ...
            <button
              ...
              className={`relative inline-flex h-5 w-9 shrink-0 rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 cursor-pointer
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 bg-white hover:bg-slate-50 transition shadow-sm cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              onClick={submit}
              disabled={!label.trim()}
              className="px-4 py-2 rounded-xl bg-linear-to-r from-blue-500 to-blue-600 text-white text-sm font-semibold hover:scale-105 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-lg shadow-blue-500/25 cursor-pointer"
            >
              Add Field
            </button>
          </div>
...
export default function SubmissionPage() {
  ...
  const visibleFields = fields.filter((f) => {
    ...
    if (selectedFilter === "Standard Fields") return !f.custom;
    if (selectedFilter === "Custom Fields") return f.custom;
    return true;
  });
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-slate-50/80">
      ...
      >
          ...
          <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
              ...
              <p className="text-slate-500 max-w-2xl ml-13">
                Configure submission fields and required information for your bootcamp.
              </p>
            </div>
            ...
            <div className="absolute top-4 right-4 z-50 animate-in slide-in-from-top-2 fade-in duration-300">
              <div className="flex items-center gap-2 px-4 py-2.5 bg-emerald-50 border border-emerald-200 rounded-xl shadow-lg">
                ...
                <span className="text-sm font-medium text-emerald-700">
                  Submission settings saved successfully!
                </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { label: "Total Fields", value: totalFields, icon: Grid3X3 },
            { label: "Visible Fields", value: visibleCount, icon: Eye },
            { label: "Mandatory Fields", value: mandatoryCount, icon: Shield },
          ].map((card, index) => {
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-slate-100 bg-linear-to-r from-slate-50/50 to-white">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-blue-50 rounded-lg">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-xs text-slate-600 hover:bg-slate-50 transition shadow-sm hover:shadow-md cursor-pointer"
                >
                  {selectedPhase}
                  <ChevronDown className="h-3 w-3 text-slate-400" />
                </button>
                {phaseOpen && (
                  <div className="absolute right-0 top-full mt-1 z-20 bg-white border border-slate-200 rounded-xl shadow-lg py-1 min-w-55">
                    {PHASES.map((p) => (
                      <button
                        key={p}
                        type="button"
                        ...
                        className={`w-full text-left px-4 py-2 text-xs hover:bg-blue-50 transition cursor-pointer
                          ${
                            selectedPhase === p
              ...
              <div className="relative">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-xs text-slate-600 hover:bg-slate-50 transition shadow-sm hover:shadow-md"
                >
                  {selectedFilter}
                  <ChevronDown className="h-3 w-3 text-slate-400" />
                </button>
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 text-xs font-medium text-blue-600 hover:bg-blue-100 transition shadow-sm border border-blue-200 cursor-pointer"
              >
          ...
          <div className="px-6">
            {visibleFields.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-slate-500">No fields match the selected filter</p>
                <p className="text-xs text-slate-400 mt-1">Try adjusting your filter selection</p>
              </div>
            ) : (
        ...
        <div className="flex items-center justify-end gap-3 pt-2 pb-8">
          {/* <button
            type="button"
            className="px-4 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 bg-white hover:bg-slate-50 transition shadow-sm hover:shadow-md cursor-pointer"
          >
            Reset to Defaults
          </button> */}
          <button
            type="button"
            onClick={handleSave}
            className={`px-5 py-2 rounded-xl text-sm font-medium shadow-lg transition-all duration-300 hover:scale-105 cursor-pointer ${
              saved
